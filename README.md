...
- **Follows Your Rules** – Enforces project-specific style and patterns via configurable templates.  
...
2. **Generate & Go** – It outputs a production-ready server with models, validation, and example-based stubs.  
...
- **Validation Built-In** – Request/response checks from the spec are auto-enforced.  
...
```sh  
npx foundry generate --spec ./api.yaml --output ./server  
```  
